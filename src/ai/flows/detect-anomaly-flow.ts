...
const DetectAnomalyOutputSchema = z.object({
  isAnomaly: z
    .boolean()
    .describe(
      "Whether or not the image contains a clear urban anomaly (like a pothole, graffiti, or broken streetlight)."
    ),
  title: z
    .string()
    .describe(
      'A concise, descriptive title for the anomaly report (e.g., "Pothole on Main Street", "Graffiti on Park Bench"). If no anomaly is detected, this should be "No Anomaly Detected".'
    ),
  description: z
    .string()
    .describe(
      "A brief, one-sentence description of the issue. If no anomaly is detected, this should state that the image appears normal."
    ),
  confidence: z
    .number()
    .describe(
      "A percentage value (0-100) indicating the confidence of the anomaly detection. If no anomaly is detected, this should be 0."
    ),
  solution: z
    .string()
    .describe(
      "A brief, one-sentence suggested solution for the detected anomaly. If no anomaly is detected, this should be an empty string."
    ),
  fixTip: z
    .string()
    .describe(
      "A brief, one-sentence practical tip for fixing the issue. If no anomaly is detected, this should be an empty string."
    ),
});
